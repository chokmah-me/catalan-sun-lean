...
| `gate_grouping.py` | **The §9 grouping gate.** Evaluates the five factor groups of (4.5) — factorial, cauchy, vandermonde, odd-linear, tail — separately, because Prop 9.5's proof gives no displayed derivation of them. `python gate_grouping.py B ...` prints the per-group table; `compare` puts the grouped route beside the direct `det R` route. Confirms the `B²log B` cancellation is structurally real, that the grouped route upper-bounds the direct one by a stable `+0.216 B²` (dropped `Ψ_A`), and reproduces the `+0.475` Prop 9.5 residue. See `FORMALIZATION-NOTES.md#grouping`. |
