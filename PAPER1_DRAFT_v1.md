...
<p class="hebrew-date" dir="rtl" lang="he">TKTK-HEBREW-DATE</p>
...
$\delta_0 > 0.00966$. Using the preprint's own equations (3.5), (5.13) and
(5.24), that assertion reduces to an inequality among three directly computable
quantities, with no appeal to the Cauchy-Binet maximum and no choice of row
set. Evaluating them at $B = 200, 400, 800, 1000, 1200$ gives $+1.826, +1.846,
+1.851, +1.854, +1.855$ for the coefficient of $B^2$, converging upward, where
...
\mathbb{Q}$. It does not claim that the approach of [1] cannot be repaired;
whether the two gaps identified below can be closed was not tested. It does not
report an error in any single line of Section 9, because Section 9's proof does
not proceed line by line. What it reports is that the quantity Theorem 9.1
...
up to the $O(B)$ contributed by the fixed factor $q^S$. Call the right side
$\mathrm{SCALAR}(B)$. Theorem 9.1 of [1] asserts exactly that
...
**Table 1.** The coefficient Theorem 9.1 requires to be below $-0.00966$,
measured at five values of $B$ with $S = \lfloor B/20\rfloor$. Increments fall
by roughly a factor of 4 per doubling of $B$, so the sequence converges to
about $+1.86$ rather than drifting logarithmically.

The value at $B = 1000$ was computed as a blind prediction test: a fit to the
first three points predicted $+1.854003$ before the run, against $+1.854020$
measured. The value at $B = 1200$ was likewise predicted at $+1.855875$ against
$+1.854615$ measured.
...
+0.477515, +0.474750, +0.475949, +0.475175$ at the five values of $B$: flat to
$\pm 0.003$ across a sixfold range. This sits in the cancellation Proposition
